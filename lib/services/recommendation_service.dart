...
class RecommendationService {
  ...
  final List<String> _filteredUserIds = [];
  Timestamp? _lastProfileUpdateChecked;

  /// Get recommended users for the current user with cursor-based pagination and action filtering
  Future<List<RecommendedUser>> getRecommendedUsers({
    ...
    try {
      if (resetCursor) {
        ...
        _filteredUserIds.clear();
      }
      // Check if profile has changed significantly or after a time period (e.g., 7 days)
      final lastUpdated = await _firebaseUserService.getLastUpdated();
      final now = Timestamp.now();
      bool shouldResetPasses = false;
      if (lastUpdated != null) {
        if (_lastProfileUpdateChecked == null ||
            lastUpdated.seconds > _lastProfileUpdateChecked!.seconds) {
          shouldResetPasses = true; // Significant profile change
        } else if (now.seconds - lastUpdated.seconds > 7 * 24 * 3600) {
          shouldResetPasses = true; // More than 7 days since last update
        }
      }
      if (shouldResetPasses) {
        await _firebaseUserService.resetPassActions();
        _lastProfileUpdateChecked = lastUpdated ?? now;
      }
      ...
      // Get action history for filtering
      final actionHistory = await _firebaseUserService
          .getActionHistoryForCurrentUser();
      final actionsTargetingMe = await _firebaseUserService
          .getActionsTargetingCurrentUser();
      final likedOrPassedUserIds = actionHistory
          .map((a) => a.targetUserId)
          .toSet();
      final passedOnMeUserIds = actionsTargetingMe
          .where((a) => a.actionType == 'pass')
          .map((a) => a.actorUserId)
          .toSet();
      final excludeSet = <String>{
        ...?excludeUserIds,
        ...likedOrPassedUserIds,
        ...passedOnMeUserIds,
        ..._filteredUserIds,
      };
      ...
      // Get all potential matches from Firestore with cursor-based pagination
      final potentialMatches = await _getPotentialMatches(
        matchingCriteria,
        limit: limit * 3, // Get more to have enough after filtering
        excludeUserIds: excludeSet.toList(),
        startAfterDoc: _lastVisibleDoc,
      );
      ...
      for (final userData in potentialMatches) {
        ...
        if (score.totalScore > 0) {
          ...
          _filteredUserIds.add(recommendedUser.userId);
        }
      }
      ...
      if (potentialMatches.isNotEmpty) {
        final lastDoc = potentialMatches.last['__docRef'] as DocumentSnapshot?;
        if (lastDoc != null) {
      ...
      print('Error in getRecommendedUsers: $e');
      throw Exception('Failed to get recommended users: $e');
    }
  }

  /// Get potential matches from Firestore with basic filtering and cursor-based pagination
  ...
  /// Get action history for filtering
  Future<List<UserAction>> getActionHistoryForFiltering() async {
    return await _firebaseUserService.getActionHistoryForCurrentUser();
  }

  /// Reset filters and pagination (e.g., when user profile changes)
  void resetFilters() {
    _lastVisibleDoc = null;
    _filteredUserIds.clear();
    _firebaseUserService.resetActionFilters();
  }
}
